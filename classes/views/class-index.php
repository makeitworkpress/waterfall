...
class Index extends Base
{
    ...
    public function header()
    {
        ...
        if (is_search() || $this->layout['header_search'] ) {
            $search_properties = is_search() ? [] : ['ajax' => true, 'types' => [wf_get_archive_post_type()]];
            $atoms['search'] = ['atom' => 'search', 'properties' => $search_properties];
        }
    ...
    public function posts()
    {
        ...
        // Adds the filter for the given posts
        if ($this->layout['content_filter'] && $this->layout['content_filter'] !== 'none') {
            $filter_args = apply_filters('waterfall_archive_filter_args', [
                'args'          => ['taxonomy' => $this->layout['content_filter']],
                'term_style'    => 'button',
                'seperator'     => ''
            ]);
            WP_Components\Build::atom('terms', $filter_args);
        }
